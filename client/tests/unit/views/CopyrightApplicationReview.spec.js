...
describe('Copyright Application Review', () => {
  it('user can review a copyright application', async () => {
    ...
    expect(wrapper.find('#possible-rights-and-permissions').find('.full-name').text()).to.equal('A B C D E')
    expect(wrapper.find('#possible-rights-and-permissions').find('.organization-name').text()).to.equal('F')
    expect(wrapper.find('#possible-rights-and-permissions').find('.address-line-1').text()).to.equal('G H')
    expect(wrapper.find('#possible-rights-and-permissions').find('.address-line-2').text()).to.equal('I, J K L')
    expect(wrapper.find('#possible-rights-and-permissions').find('.phone-number').text()).to.equal('M')
    expect(wrapper.find('#possible-rights-and-permissions').find('.email').text()).to.equal('N')
  ...
  it('user can review a copyright application authors', async () => {
    ...
    expect(wrapper.find('#author-111').find('.full-name').text()).to.equal('Author:Mr John Rad Wick III')
    expect(wrapper.find('#author-111').find('.pseudonym').text()).to.equal('Pseudonym:Neo')
    expect(wrapper.find('#author-111').find('.citizenship').text()).to.equal('Citizenship:ZION')
    expect(wrapper.find('#author-111').find('.domicile').text()).to.equal('Domicile:STARS')
    expect(wrapper.find('#author-111').find('.year-of-birth').text()).to.equal('Year of Birth:1988')
    expect(wrapper.find('#author-111').find('.deceased').text()).to.equal('Deceased:1999')
    expect(wrapper.find('#author-111').find('.work-made-for-hire').text()).to.equal('Work Made For Hire:No')

    expect(wrapper.find('#author-222').find('.organization-name').text()).to.equal('Organization Name:Amazon')
    expect(wrapper.find('#author-222').find('.work-made-for-hire').text()).to.equal('Work Made For Hire:Yes')
    expect(wrapper.find('#author-222').find('.domicile').text()).to.equal('Domicile:MARS')
  })
  ...
  it('user can review a copyright application claimants', async () => {
    ...
    expect(wrapper.find('#claimant-333').find('.full-name').text()).to.equal('Ms Jane Johns Jacobs II')
    expect(wrapper.find('#claimant-333').find('.address-line-1').text()).to.equal('123 This Ln apt 423')
    expect(wrapper.find('#claimant-333').find('.address-line-2').text()).to.equal('Baltimore, TX 32312 USA')

    expect(wrapper.find('#claimant-444').find('.organization-name').text()).to.equal('DELL')
  })
})
